...
- I described the task as one of predicting the final location given the previous stops in a trajectory sequence. My first clue that something was wrong with the model was very fast loss convergence and excellent performance on test datasets. Upon closer inspection, I realized that the model was actually predicting the final location from the full trajectory (i.e. including the final location).
- The model also initially treated the user_id variable as a continuous variable instead of a categorical variable. When I pointed out the mistake Claude had a good suggestion to use an embedding layer.
- For the dashboard I analyze the first two PCA dimensions of the trajectory embeddings. When describing this step I asked Claude to normalize the matrix before applying PCA. The normalization algorithm that it selected was a matrix L2 norm instead of the traditional zero-mean, unit-variance normalization method. I was able to ask it directly to switch and it chose the correct method.